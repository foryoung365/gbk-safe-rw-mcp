...
function buildDenyReason({
  ...
  const recentTimes = recentRecords
    .map(record => new Date(record.timestamp).toISOString())
    .join(', ')
  return [
...
function getContextInfo(input: HookInput): string {
  ...
  const fromTranscript = transcriptPath
    ? findContextInfoInTranscript(transcriptPath)
    : undefined
...
function findContextInfoInTranscript(transcriptPath: string): ContextNumbers | undefined {
  try {
...
function findTokenUsageBlock(value: unknown): Omit<ContextNumbers, 'source'> | undefined {
  const found = findObject(value, object =>
    object.type === 'token_usage' &&
    typeof object.total === 'number' &&
    typeof object.remaining === 'number',
  )
  if (!found) return undefined
  const total = found.total as number
  const remaining = found.remaining as number
  return {
...
function findContextNumbers(value: unknown): ContextNumbers | undefined {
  const found = findObject(value, object => {
...
function countTrailingSignatureMatches(
  records: ToolCallRecord[],
  signature: string,
): number {
  let count = 0
...
function getStatePath(input: HookInput): string {
  const root = process.env.SAFE_RW_LOOP_GUARD_STATE_DIR ||
    path.join(os.tmpdir(), 'safe-read-write-mcp', 'tool-loop-guard')
  const session = input.session_id || 'unknown-session'
  const cwd = input.cwd || process.cwd()
  const agent = input.agent_id || input.agent_type || 'main'
  const key = createHash('sha256')
    .update(`${session}\0${cwd}\0${agent}`)
    .digest('hex')
  return path.join(root, `${key}.json`)
}

async function readState(filePath: string): Promise<LoopGuardState> {
  try {
    ...
    if (
      ...
      return {
        version: 1,
        records: (parsed as { records: ToolCallRecord[] }).records.filter(isRecord),
      }
    }
  } catch {
...
async function writeState(filePath: string, state: LoopGuardState): Promise<void> {

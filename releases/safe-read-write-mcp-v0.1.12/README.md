...
```text
D:/tools/safe-read-write-mcp-v0.1.12
```

2. 在需要启用该 MCP 的仓库根目录执行：

```bash
node D:/tools/safe-read-write-mcp-v0.1.12/install.mjs .
```

也可以显式指定目标仓库：

```bash
node D:/tools/safe-read-write-mcp-v0.1.12/install.mjs D:/your/repo
```
